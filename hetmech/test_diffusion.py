...
from .diffusion import degree_weight_adjacency_matrix as dwam
...
class TestDualNormalize:
    ...
    def test_dwam_row_or_column_damping(self, exponent):
        ...
        matrix = dwam(input_matrix, row_damping=exponent)
        assert numpy.allclose(expect, matrix)

        # Test column normalization works as expected
        input_matrix = self.get_clean_matrix()
        matrix = dwam(input_matrix, column_damping=exponent)
    ...
    def test_dwam_row_and_column_damping(
